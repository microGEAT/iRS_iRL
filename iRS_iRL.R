...
row.names(my.dat.fra) <- c("Shannon","other1","other2")
#we need  a factor
...
out<- fun.RS.RL.BOOT.2(shannon, "Shannon") # we need the out structure
for (i in 1:3) { 
...
fun.RS.RL.BOOT.2<-function(y,tit){
  ...
  for(ii in 1:4){
    ...
    PrB<-pnorm(1,meB,sdB,lower.tail = F)
...
fun.clasif<-function(cc, rr){  #  (P.RS, P.Cx-Co, P.RL, iRL, Po-Co)
  pert<-ifelse (rr > 1 ,"+",  "-") ; myclasif= "RS"
  if (cc[1] >= .05) {myclasif= "RS";  pert =""}
  else {
    if (cc[2] < 0.05) {myclasif= "TV"}
    else{
...
fun.RS.RL<-function(Co,Cx,Po,Px){ 
  ...
  if (mCo == 0 & mPo == 0) {RS=1; RL.OW=1; RL =1}
